...
def masked_bce_loss(outputs, labels, mask):
    """
    Compute masked binary cross-entropy loss.
    
    Args:
        outputs: model outputs [batch_size, seq_len, 1]
        labels: ground truth labels [batch_size, seq_len]
        mask: valid positions mask [batch_size, seq_len]
    
    Returns:
        float: masked BCE loss
    """
    outputs = outputs.squeeze(-1)
    loss = nn.BCEWithLogitsLoss(reduction='none')(outputs, labels.float())
...
def main(args):
    """
    Main training function with all top conference standards.
    """
    # 设置随机种子（论文级必须）
    set_seed(args.seed)
    
    print(f"Training TSAKT-Linear (No Positional Encoding) on {args.dataset}")
    ...
    model = TSAKT_Linear_NoPos(num_items, num_skills, embed_size=args.embed_size,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train TSAKT-Linear WITHOUT Positional Encoding (Top Conference Standards)')
    parser.add_argument('--dataset', type=str, default='assistments12')
    parser.add_argument('--savedir', type=str, default='save/tsakt-linear-nopos')
