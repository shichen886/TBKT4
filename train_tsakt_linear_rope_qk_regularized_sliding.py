...
def main(args):
    ...
    # Early stopping with sliding window
    best_val_auc = 0
    best_sliding_avg = 0
    patience_counter = 0
    best_epoch = 0
    ...
    print(f'Sliding window early stopping: window_size={args.window_size}, patience={args.patience}')
    for epoch in range(args.num_epochs):
        ...
        # Update validation AUC history
        val_auc_history.append(val_auc)
        
        # Calculate sliding average
        if len(val_auc_history) >= args.window_size:
            sliding_avg = np.mean(val_auc_history[-args.window_size:])
        else:
            sliding_avg = val_auc
        
        history['val_sliding_avg'].append(sliding_avg)
        
        print(f'Train Loss: {train_loss:.4f}, Train AUC: {train_auc:.4f}')
        print(f'Val Loss: {val_loss:.4f}, Val AUC: {val_auc:.4f}, Val RMSE: {val_rmse:.4f}')
        print(f'Val Sliding Avg (window={args.window_size}): {sliding_avg:.4f}')
        
        # Learning rate scheduling
        scheduler.step(val_auc)
        
        # Early stopping based on sliding average
        if sliding_avg > best_sliding_avg:
            best_sliding_avg = sliding_avg
            ...
            print(f'New best model saved! Val Sliding Avg: {best_sliding_avg:.4f}, Val AUC: {best_val_auc:.4f}')
        else:
            patience_counter += 1
            if patience_counter >= args.patience:
                print(f'Early stopping at epoch {epoch + 1}')
                print(f'Reason: Sliding average has not improved for {args.patience} epochs')
                break
    ...
    with open(config_path, 'w') as f:
        ...
        config['best_sliding_avg'] = best_sliding_avg
        config['best_epoch'] = best_epoch
        config['generalization_gap'] = best_val_auc - test_auc
        json.dump(config, f, indent=4)
    
    print(f'\nTraining completed!')
    print(f'Best Val Sliding Avg: {best_sliding_avg:.4f}')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train TSAKT-Linear with RoPE on Q/K (Increased Regularization + Sliding Window Early Stopping)')
    parser.add_argument('--dataset', type=str, default='assistments12')
    parser.add_argument('--savedir', type=str, default='save/tsakt-linear-rope-qk-regularized-sliding')
