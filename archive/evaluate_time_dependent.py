...
# 评估TSAKT-Ful（带位置编码）
tsakt_ful_path = os.path.join('save', 'tsakt-ful-time', f'{dataset},batch_size=128,max_length={max_length},encode_pos=True,max_pos={max_length},tensor_rank=3')
if os.path.exists(tsakt_ful_path):
    print(f"\nEvaluating TSAKT-Ful on {dataset}...")
...
tsakt_wo_pos_path = os.path.join('save', 'tsakt-wo-pos-time', f'{dataset},batch_size=128,max_length={max_length},encode_pos=True,max_pos=5,tensor_rank=3')
...
if tsakt_ful_results and tsakt_wo_pos_results:
    print(f"\n{'-' * 60}")
    print(f"Time-Dependent Ablation Study Results for {dataset}:")
    print(f"{'-' * 60}")
    ...
    with open('time_dependent_ablation_results.json', 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    
    print(f"\nResults saved to time_dependent_ablation_results.json")

print("\n" + "=" * 80)
print("Time-Dependent Ablation Study Completed!")
print("=" * 80)