...
# 实现缩放点积注意力机制
def attention(query, key, value, score_glo, mask=None, dropout=None):
...
# 实现了一个带有相对位置编码的自注意力机制
def relative_attention(query, key, value, pos_key_embeds, pos_value_embeds, score_glo, mask=None, dropout=None):
    ...
    scores = torch.matmul(query, key.transpose(-2, -1)) + score_glo  # 矩阵乘法
...
# 实现了一个多头自注意力层，能够处理输入序列的不同位置之间的关系，并通过多头机制捕获不同类型的信息
class MultiHeadedAttention(nn.Module):
    def __init__(self, total_size, num_heads, drop_prob):
        ...
        self.linear_global = nn.Linear(total_size, num_heads)
    ...
    def forward(self, query, key, value, encode_pos, pos_key_embeds, pos_value_embeds, mask=None):
        ...
        if encode_pos:
            out, self.prob_attn = relative_attention(
                query, key, value, pos_key_embeds, pos_value_embeds, score_glo, mask, self.dropout)
        else:
            out, self.prob_attn = attention(query, key, value, score_glo, mask, self.dropout)
...
class SAKT(nn.Module):
    ...
    def forward(self, item_inputs, skill_inputs, label_inputs, item_ids, skill_ids):
        ...
        weight = torch.matmul(outputs, outputs.transpose(1, 2))
        weight = F.softmax(weight.masked_fill(mask, -1e9), dim=-1)
        ...
        return self.lin_out(outputs), weight