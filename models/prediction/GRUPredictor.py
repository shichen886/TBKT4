...
class GRUPredictor:
    ...
    def __init__(self):
        ...
        # GRU权重
        # 更新门
        self.W_z = np.random.randn(3, self.hidden_dim) * 0.01
        self.U_z = np.random.randn(self.hidden_dim, self.hidden_dim) * 0.01
        self.b_z = np.zeros(self.hidden_dim)
        
        # 重置门
        self.W_r = np.random.randn(3, self.hidden_dim) * 0.01
        self.U_r = np.random.randn(self.hidden_dim, self.hidden_dim) * 0.01
        self.b_r = np.zeros(self.hidden_dim)
        
        # 候选隐藏状态
        self.W_h = np.random.randn(3, self.hidden_dim) * 0.01
        self.U_h = np.random.randn(self.hidden_dim, self.hidden_dim) * 0.01
    ...
    def _forward(self, X):
        ...
        for t in range(seq_len):
            # 更新门
            z_t = self._sigmoid(np.dot(X[t], self.W_z) + np.dot(h[t], self.U_z) + self.b_z)
            # 重置门
            r_t = self._sigmoid(np.dot(X[t], self.W_r) + np.dot(h[t], self.U_r) + self.b_r)
            # 候选隐藏状态
            h_tilde = np.tanh(np.dot(X[t], self.W_h) + np.dot(r_t * h[t], self.U_h) + self.b_h)
            # 更新隐藏状态
            h[t+1] = (1 - z_t) * h[t] + z_t * h_tilde
            # 计算输出
    ...
    def predict(self, features, labels, n_future=3):
        ...
        try:
            ...
            print(f"GRU模型原始预测(前5个): {model_predictions[:5]}")
        ...
        for i in range(n_future):
            ...
            try:
                # 更新门
                z_t = self._sigmoid(np.dot(x_future, self.W_z) + np.dot(last_h, self.U_z) + self.b_z)
                # 重置门
                r_t = self._sigmoid(np.dot(x_future, self.W_r) + np.dot(last_h, self.U_r) + self.b_r)
                # 候选隐藏状态
                h_tilde = np.tanh(np.dot(x_future, self.W_h) + np.dot(r_t * last_h, self.U_h) + self.b_h)
                # 更新隐藏状态
                next_h = (1 - z_t) * last_h + z_t * h_tilde
                # 计算输出
                model_pred = self._sigmoid(np.dot(next_h, self.W_y) + self.b_y)
                
                # 添加随机扰动
                pred = model_pred[0] + random.uniform(-0.15, 0.15)
                # 确保预测值在合理范围内
                ...
                if pred > 0.5:
                    x_future[2] *= 0.94  # 假设回答正确，响应时间略微减少
                else:
                    x_future[2] *= 1.06  # 假设回答错误，响应时间略微增加
            except Exception as e:
                print(f"未来预测计算错误: {e}")
                # 出错时基于趋势和随机性生成预测
                if recent_avg > 0.6:
                    pred = recent_avg + random.uniform(-0.12, 0.08)
                elif recent_avg < 0.4:
                    pred = recent_avg + random.uniform(-0.08, 0.12)
                else:
                    pred = recent_avg + random.uniform(-0.15, 0.15)
        ...
        print(f"GRU最终预测范围: 最小值={min(all_preds):.4f}, 最大值={max(all_preds):.4f}")
        print(f"----- GRU预测调试信息结束 -----\n")
